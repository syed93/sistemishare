...
{{ partial("partials/user_left") }}
	<div class="jun_right line">
		<div class="jform_post jun_view">
			...
			{{ link_to("messages/index", "Inbox", "class": "jun_button") }}{{ link_to("messages/sentitems", "Outbox", "class": "jun_button") }}
			<div class="jun_messages">
				{% for post in views %} 
				
				{{ link_to("profile/" ~ post.username, image('uploads/profiles/'~post.image) ~ post.username) }}</p></p>{{read_date}} {{read_time}}</p>
				<pre>{{post.body}}</pre>
				{% endfor %}

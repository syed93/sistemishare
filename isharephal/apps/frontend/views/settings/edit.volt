  ...
  <div class="jform_post">
      ...
      {{ form('users/login', 'method': 'post') }}
	   ...
	   <h2>Vehicle Information</h2><li>{{link_to('settings/edit/', image('img/icons/edit.png'), 'title': 'Edit Profile')}}</li>   
	      ...
	      <label><p>Due Date</p>: {{ text_field("due_date", "size": 30, "value": user.insuran_due_date) }}</label>
	      
	      <label><p>Reg No</p>: {{ text_field("reg_no", "size": 30, "value": user.reg_number) }}</label>
